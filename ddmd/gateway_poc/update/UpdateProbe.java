...
public final class UpdateProbe {
 static Object lookup(AuthToken token)throws Exception {
  ...
  DdmdMessage request=new DdmdMessage();request.setAction("CLT_MOD_UPT_REQ");
  ...
  for(Map.Entry<ModUpdInf,String> entry:update.getModUpdInfs().entrySet()){
   ModUpdInf m=entry.getKey();rows.add(m.getModId()+"\t"+m.getModVer()+"\t"+m.getFndtPth()+"\t"+m.getChecksum());
  }
  java.nio.file.Files.write(HeadlessGateway.base.resolve("update-modules.tsv"),rows,java.nio.charset.StandardCharsets.UTF_8);
 ...
 public static void main(String[] args){try{
  ...
  HeadlessGateway.REPORT.println("RESULT GATEWAY_PASS mode=update headless=true requests="+HeadlessGateway.requestCount);
 }catch(Throwable e){
