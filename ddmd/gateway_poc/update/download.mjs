...
try {
 ...
 state.args=state.args.map(a=>a.replace('-Djdk.serialFilter=','-Dhira.update.serialFilter=').replace('maxbytes=1048576;','maxbytes=268435456;').replace('kr.co.bizframe.msi.*;','kr.co.bizframe.msi.*;kr.co.bizframe.msi.registry.AttachmentReference;')); state.args[state.args.length-2]='UpdateDownloadProbe'; const child=spawn(state.java,state.args,{cwd:state.cwd,env:process.env,stdio:['pipe','pipe','pipe']});
 ...
 const passed=code===0&&lines.includes('RESULT GATEWAY_PASS mode=update-download headless=true requests=2');
 ...
 fs.writeFileSync(new URL('../evidence/macos-update-download-result.json',import.meta.url),JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report,null,2));if(!passed)process.exitCode=1;
