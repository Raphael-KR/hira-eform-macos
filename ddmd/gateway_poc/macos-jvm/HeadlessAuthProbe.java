...
public final class HeadlessAuthProbe {
  ...
  public static void main(String[] args)throws Exception {
    ...
    byte[] payload=new byte[65537];for(int n=0;n<payload.length;n++)payload[n]=(byte)(n*37);
    SignedDataCipher signed=provider.openSignedDataCipher();signed.initSign(info);
    byte[] cms=signed.sign(payload);ok(Arrays.equals(payload,provider.openSignedDataCipher().verify(cms).getContent()),"binary payload CMS sign and verify");
    Files.write(root.resolve("file-signed.cms"),cms);Files.write(root.resolve("file-original.bin"),payload);
    EnvelopedDataCipher fileEnc=provider.openEnvelopedDataCipher();fileEnc.initEncrypt(cert.getEncoded());
    fileEnc.process(root.resolve("file-original.bin").toFile(),root.resolve("file-encrypted.cms").toFile());
    EnvelopedDataCipher fileDec=provider.openEnvelopedDataCipher();fileDec.initDecrypt(cert.getEncoded(),pk8);
    fileDec.process(root.resolve("file-encrypted.cms").toFile(),root.resolve("file-decrypted.bin").toFile());
    ok(Arrays.equals(payload,Files.readAllBytes(root.resolve("file-decrypted.bin"))),"file encryption and decryption preserve binary bytes");
    byte[] damaged=cms.clone();damaged[damaged.length/2]^=1;boolean bad=false;
    try{SignedDataInfo tested=provider.openSignedDataCipher().verify(damaged);bad=!Arrays.equals(payload,tested.getContent());}catch(Exception e){bad=true;}
    ok(bad,"tampered CMS is rejected or fails exact content check");
    ok(guard.blocked==expectedBlocks,"file crypto uses no extra native GUI or network");
    hira.ddmd.jmc.common.DdmdMessage message=new hira.ddmd.jmc.common.DdmdMessage();
    message.setAction("SYNTHETIC_OFFLINE_ONLY");message.setBody(payload);message.addFile("fixture",root.resolve("file-original.bin").toFile());
    DdmdMessageSignature msgSigner=provider.openDdmdMessageSignature("SHA-256");
    byte[] msgSignature=msgSigner.sign(message,info);
    Files.write(root.resolve("message-signed.cms"),msgSignature);
    ok(msgSignature.length>0,"DDMD message attachment signature generated offline");
    try{msgSigner.verify(message,msgSignature);System.out.println("BOUNDARY messageVerifier=PASS");}
    catch(Exception e){System.out.println("BOUNDARY messageVerifier="+e.getClass().getName()+" cause="+(e.getCause()==null?"none":e.getCause().getClass().getName()));}
