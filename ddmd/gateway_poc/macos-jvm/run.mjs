...
const mode=process.argv[2];if(!['auth','lifecycle','recipient','failure-tests','notice-list','download'].includes(mode)){console.error('INVALID_COMMAND');process.exit(2);}
let setupStage="prepared";
try {
 const state=JSON.parse(fs.readFileSync(new URL('../evidence/macos-live-prepared.json',import.meta.url)));
 const hash=crypto.createHash('sha256').update(fs.readFileSync(new URL('./HeadlessGateway.java',import.meta.url))).digest('hex');
 if(hash!==state.sourceSha256||state.args.at(-1)!=='auth')throw Error('prepared mismatch');
 ...
 let km='';if(mode==='download'){const base=os.homedir()+'/Library/Preferences/NPKI/KICA/USER';const dirs=fs.readdirSync(base,{withFileTypes:true}).filter(e=>e.isDirectory()).map(e=>base+'/'+e.name).filter(d=>fs.existsSync(d+'/kmCert.der'));if(dirs.length!==1)throw Error('km certificate selection');km=fs.readFileSync(dirs[0]+'/kmCert.der').toString('base64')+'\n';}
 ...
 const args=[...state.args];args[args.length-1]=mode;if(mode==='recipient'){if(crypto.createHash('sha256').update(fs.readFileSync(new URL('./RecipientProbe.java',import.meta.url))).digest('hex')!==state.recipientSha256)throw Error('recipient mismatch');args[args.length-2]='RecipientProbe';}
 if(mode==='notice-list'){if(crypto.createHash('sha256').update(fs.readFileSync(new URL('./NoticeListProbe.java',import.meta.url))).digest('hex')!==state.noticeSha256)throw Error('notice mismatch');args[args.length-2]='NoticeListProbe';}
 if(mode==='download'){if(crypto.createHash('sha256').update(fs.readFileSync(new URL('./DownloadProbe.java',import.meta.url))).digest('hex')!==state.downloadSha256)throw Error('download mismatch');const parent=new URL('../private-downloads/',import.meta.url);fs.mkdirSync(parent,{recursive:true,mode:0o700});const output=fs.mkdtempSync(new URL('notice-',parent).pathname);args[args.length-2]='DownloadProbe';args.unshift('-Dhira.download=true','-Dhira.output='+output);console.log('DOWNLOAD_DIRECTORY '+output);}
 const child=spawn(state.java,args,{cwd:state.cwd,env:process.env,stdio:['pipe','pipe','pipe']});
 ...
 const passed=code===0&&lines.some(s=>s.startsWith('RESULT GATEWAY_PASS mode='+mode+' headless=true requests='));
 const report={startedAtUtc,finishedAtUtc:new Date().toISOString(),mode,sourceSha256:hash,runtime:'macOS ARM64 Zulu Java 8u504 native JVM',exitCode:code,passed,stderrBytes,lines};
 fs.writeFileSync(new URL('../evidence/macos-'+mode+'-result.json',import.meta.url),JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report,null,2));if(!passed)process.exitCode=1;
