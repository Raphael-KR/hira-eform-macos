...
public final class HeadlessGateway {
  ...
  static String mode;
  static X509Certificate cert,center,downloadCert;
  static String ykiho;
  static byte[] random;
  ...
  static void initialize()throws Exception {
    ...
    Properties config=new Properties();try(InputStream in=Files.newInputStream(base.resolve("ddmd.properties"))){config.load(in);}
    BufferedReader input=new BufferedReader(new InputStreamReader(System.in,"US-ASCII"));
    ykiho=input.readLine();require(ykiho!=null,"institution input missing");ykiho=ykiho.trim();require(ykiho.matches("[0-9]{8}")&&!ykiho.equals("00000000"),"institution missing");
    require(ENDPOINT.equals(config.getProperty("msi.endpoint","").trim()),"endpoint mismatch");
    CertificateFactory cf=CertificateFactory.getInstance("X.509");
    try(InputStream in=Files.newInputStream(base.resolve("kmCert.der"))){center=(X509Certificate)cf.generateCertificate(in);}center.checkValidity();
    ...
    if(Boolean.getBoolean("hira.download")){String line=input.readLine();require(line!=null&&line.length()<20000,"download certificate missing");downloadCert=(X509Certificate)cf.generateCertificate(new ByteArrayInputStream(Base64.getDecoder().decode(line)));downloadCert.checkValidity();}
