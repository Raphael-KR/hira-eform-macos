...
root=Path(__file__).resolve().parents[1];source=Path(os.environ.get('DDMD_SOURCE_DIR',''))
...
trial=Path(tempfile.mkdtemp(prefix='hira-live-auth-macos-',dir='/private/tmp/hira-jvm-trial'))
...
hashes={}
for lib in libs:
 src=source/lib;dst=trial/'lib'/src.name;shutil.copy2(src,dst);hashes[src.name]=hashlib.sha256(dst.read_bytes()).hexdigest()
