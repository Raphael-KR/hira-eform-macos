...
public final class RecipientProbe {
 static Object lookup(AuthToken token)throws Exception {
  HeadlessGateway.require(!token.isExpired(),"expired token");
  RecpHbrReq body=new RecpHbrReq();body.setYkiho(HeadlessGateway.ykiho);
  body.setEamUgrpCode((String)token.getAttribute("eamUgrpCode"));
  String date=new java.text.SimpleDateFormat("yyyyMMdd").format(new Date());
  body.setAdtFrDd(date);body.setAdtEndDd(date);body.setInsuTpCd("Y");
  ...
  DdmdMessage request=new DdmdMessage();request.setAction("CLT_CERT_REQ");
  ...
  HeadlessGateway.stage="recipient-query";HeadlessGateway.requestCount++;
  ...
  Collection recipients=(Collection)result;HeadlessGateway.require(!recipients.isEmpty(),"empty recipients");
  for(Object item:recipients)HeadlessGateway.require(item instanceof RecpHbrRes,"unexpected recipient item");
  HeadlessGateway.REPORT.println("CHECK recipientQuery=true tokenHeaders=true recipientCount="+recipients.size()+" authenticatedResponse="+authenticatedResponse+" attachments=0");
  return result;
 }
 public static void main(String[] args){try{
  ...
  HeadlessGateway.REPORT.println("RESULT GATEWAY_PASS mode=recipient headless=true requests="+HeadlessGateway.requestCount);
 }catch(Throwable e){
