...
subprocess.run(['scp','-q',str(root/'live/LiveAuth.java'),str(compiler),'user@192.168.219.146:'+remote+'/'],check=True)
s="Set-Location '"+rw+"';$cp=(Get-ChildItem '.\\lib\\*.jar').FullName -join ';'; & 'C:\\hira\\DDMD\\jre18\\bin\\java.exe' -jar .\\ecj-3.26.0.jar -1.8 -proc:none -classpath $cp -d .\\classes .\\LiveAuth.java;if($LASTEXITCODE -ne 0){throw 'compile failed'};'COMPILE PASS'"
print(ps(s))
# Global Java serialization limits; only credential/token DTOs and basic Java data are accepted.
fil='maxdepth=32;maxrefs=10000;maxbytes=1048576;maxarray=1048576;java.lang.*;java.util.*;java.security.cert.*;javax.crypto.spec.SecretKeySpec;hira.ddmd.jmc.agent.auth.*;hira.ddmd.jmc.common.dto.*;hira.ddmd.jmc.common.DdmdException;kr.co.bizframe.msi.*;!*'
# cmd.exe consumes stdin directly; no PowerShell stdin read ambiguity with secret pipes.
cmd='cd /d "'+rw+'" && "C:\\hira\\DDMD\\jre18\\bin\\java.exe" -Djava.awt.headless=true "-Djava.io.tmpdir='+rw+'\\javatmp" "-Djdk.serialFilter='+fil+'" -cp "classes;lib/*" LiveAuth'
state={'remote':remote,'command':cmd,'sourceSha256':hashlib.sha256((root/'live/LiveAuth.java').read_bytes()).hexdigest(),'preparedAtUtc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'realAuthSent':False}
(root/'evidence/live-prepared.json').write_text(json.dumps(state,indent=2)+'\n')
