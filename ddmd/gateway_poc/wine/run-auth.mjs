// Fixed authentication-only Wine trial. Never persists identity or response payloads.
...
try {
 const state=JSON.parse(fs.readFileSync(new URL('../evidence/wine-live-prepared.json',import.meta.url)));
 ...
 setupStage="wine-spawn";
 const startedAtUtc=new Date().toISOString();
 const child=spawn(state.wine,state.args,{cwd:state.cwd,env:{...process.env,WINEPREFIX:state.prefix,WINEDEBUG:'-all',MVK_CONFIG_LOG_LEVEL:'0',WINEDLLOVERRIDES:'mscoree,mshtml='},stdio:['pipe','pipe','pipe']});
 ...
 const passed=code===0&&lines.includes('RESULT GATEWAY_PASS mode=auth headless=true requests=1');
 const report={startedAtUtc,finishedAtUtc:new Date().toISOString(),sourceSha256:hash,runtime:'Wine 11.0 Windows Java 8u171 -Xint',exitCode:code,passed,stderrBytes,lines};
 fs.writeFileSync(new URL('../evidence/wine-live-auth-result.json',import.meta.url),JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report,null,2));if(!passed)process.exitCode=1;
}catch{console.error('WINE_AUTH_SETUP_FAILED stage='+setupStage);process.exitCode=1;}
