...
state=json.loads((root/'evidence/gateway-prepared.json').read_text())
...
@{sshSessionId=(Get-Process -Id $PID).SessionId;osVersion=[Environment]::OSVersion.Version.ToString();
 sourceSha256=(Get-FileHash -Algorithm SHA256 -LiteralPath ($r+'\HeadlessGateway.java')).Hash;
...
(root/'evidence/gateway-environment.json').write_text(json.dumps(data,indent=2)+'\n')
